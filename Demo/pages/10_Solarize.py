...
st.set_page_config(page_title="Solarize Demo", page_icon="📈")

st.markdown("# Solarize")
st.sidebar.header("Image selection")
st.write(
    """This demo shows the effects of the `Solarize` transformation and its parameters.
    Enjoy!"""
)
...
limit1 = st.sidebar.slider('Threshold?', 0, 255, value=128)
limit2 = st.sidebar.slider('Probability of applying the transform?', 0.5, 1.0, value=0.7)


@st.cache
def get_transformation(limit1: int, limit2: float):
    return A.Solarize(threshold=limit1, p=limit2)
