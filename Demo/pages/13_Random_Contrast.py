...
limit = st.sidebar.slider('Factor range for changing contrast?', 0.0, 3.0, value=2.0)
p = st.sidebar.slider('Probability of applying the transform?', 0.0, 1.0, value=0.5)

@st.cache
def get_transformation(limit: float, p: float):
    return A.RandomContrast(limit=limit, always_apply=True, p=p)
...
def run():
    ...
    transformation = get_transformation(limit=limit, p=p)
