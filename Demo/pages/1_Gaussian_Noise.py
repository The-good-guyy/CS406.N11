...
var_limit_lower = st.sidebar.slider('Variance (lower bound)?', 100, 100000, value=1000)
var_limit_upper = st.sidebar.slider('Variance (upper bound)?', 100, 100000, value=10000)
mean = st.sidebar.slider('Mean?', 0, 255, value=100)
per_channel = st.sidebar.checkbox(label="Apply per channel?", value=False)


@st.cache
def get_transformation(var_limit_lower: int, var_limit_upper: int, mean: int, per_channel: bool):
...
def plot_original_image(img, additional_information=None):
    st.markdown(
        f"<h4 style='text-align: center; color: white;'>Original</h5>",
...
def plot_modified_image(img):
    st.markdown(
        f"<h4 style='text-align: center; color: white;'>Augmented image</h5>",
        unsafe_allow_html=True,
    )
    st.image(img, use_column_width=True)
